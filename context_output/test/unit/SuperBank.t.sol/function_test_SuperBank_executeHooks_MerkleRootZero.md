# Function: test_SuperBank_executeHooks_MerkleRootZero()
...
function test_SuperBank_executeHooks_MerkleRootZero() public {
    ...
    vm.mockCall(address(superGovernor), abi.encodeWithSignature("getSuperBankHookMerkleRoot(address)", address(mockHook)), abi.encode(bytes32(0)));
...
```
┌─ [0] ⚙️ FUNCTION: SuperBankTest.test_SuperBank_executeHooks_MerkleRootZero() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
```
