# Function: test_SuperBank_SwapHookOdos()
...
function test_SuperBank_SwapHookOdos() public {
    ...
    bytes memory odosCalldata = abi.encodePacked(address(token), amount, address(this), address(up), amount, amount - ((amount * 1e4) / 1e5), true, uint256(0), bytes(""), address(0), uint32(0), false);
    ...
    expectedOutputs[0] = 0;
    expectedOutputs[1] = 0;
    IHookExecutionData.HookExecutionData memory executionData = IHookExecutionData.HookExecutionData({hooks: hooksAddresses, data: hooksData, merkleProofs: merkleProofs, expectedAssetsOrSharesOut: expectedOutputs});
    ...
    vm.mockCall(address(superGovernor), abi.encodeWithSignature("getSuperBankHookMerkleRoot(address)", hooksAddresses[1]), abi.encode(merkleRoot2));
...
```
┌─ [0] ⚙️ FUNCTION: SuperBankTest.test_SuperBank_SwapHookOdos() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
  ├─ [1] ⚙️ FUNCTION: Helpers._getTokens(address,address,uint256) (NodeID: 1)
  │   💬 Args: [address(token), address(superBank), amount]
  │   👁️  Def: internal
  ...
  │               💬 Args: [self]
  │               👁️  Def: internal
  ├─ [1] ⚙️ FUNCTION: InternalHelpers._createApproveHookData(address,address,uint256,bool) (NodeID: 100)
  │   💬 Args: [address(token), address(odosRouter), amount, false]
  │   👁️  Def: internal
  ├─ [1] ⚙️ FUNCTION: StdAssertions.assertGt(uint256,uint256) (NodeID: 101)
  │   💬 Args: [inputTokenBalanceBefore, inputTokenBalanceAfter]
  │   👁️  Def: internal
  └─ [1] ⚙️ FUNCTION: StdAssertions.assertGt(uint256,uint256) (NodeID: 102)
      💬 Args: [upTokenBalanceAfter, upTokenBalanceBefore]
      👁️  Def: internal
```
