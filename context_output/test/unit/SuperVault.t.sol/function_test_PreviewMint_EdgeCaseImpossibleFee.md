# Function: test_PreviewMint_EdgeCaseImpossibleFee()
...
function test_PreviewMint_EdgeCaseImpossibleFee() public {
    vm.prank(manager);
    (address vaultAddress, , ) = superVaultAggregator.createVault(ISuperVaultAggregator.VaultCreationParams({asset: address(asset), name: "Test Vault 100% Fee", symbol: "TV100", mainManager: manager, secondaryManagers: new address[](0), minUpdateInterval: 5, maxStaleness: 300, feeConfig: ISuperVaultStrategy.FeeConfig({performanceFeeBps: 0, managementFeeBps: 10_000, recipient: manager})}));
...
```
┌─ [0] ⚙️ FUNCTION: SuperVaultTest.test_PreviewMint_EdgeCaseImpossibleFee() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
  ...
  │   💬 Args: [grossAssets, 0, "Should return 0 when feeBps >= BPS_PRECISION (impossible to mint)"]
  │   👁️  Def: internal
  ├─ [1] ⚙️ FUNCTION: StdAssertions.assertEq(uint256,uint256,string) (NodeID: 101)
  │   💬 Args: [testVault.previewMint(1), 0, "Should return 0 for any amount when fee is 100%"]
  │   👁️  Def: internal
  ├─ [1] ⚙️ FUNCTION: StdAssertions.assertEq(uint256,uint256,string) (NodeID: 102)
  │   💬 Args: [testVault.previewMint(1000e18), 0, "Should return 0 for any amount when fee is 100%"]
  │   👁️  Def: internal
  └─ [1] ⚙️ FUNCTION: StdAssertions.assertEq(uint256,uint256,string) (NodeID: 103)
      💬 Args: [testVault.previewMint(type(uint256).max), 0, "Should return 0 for any amount when fee is 100%"]
      👁️  Def: internal
```
