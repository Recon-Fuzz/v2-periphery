# Function: test_CreateVault_RevertEmptySymbol()
...
function test_CreateVault_RevertEmptySymbol() public {
    ...
    superVaultAggregator.createVault(ISuperVaultAggregator.VaultCreationParams({asset: address(asset), name: "Test Vault", symbol: "", mainManager: manager, secondaryManagers: new address[](0), minUpdateInterval: 5, maxStaleness: 300, feeConfig: ISuperVaultStrategy.FeeConfig({performanceFeeBps: 1000, managementFeeBps: 0, recipient: manager})}));
}
```
...
```
┌─ [0] ⚙️ FUNCTION: SuperVaultAggregatorTest.test_CreateVault_RevertEmptySymbol() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
```
