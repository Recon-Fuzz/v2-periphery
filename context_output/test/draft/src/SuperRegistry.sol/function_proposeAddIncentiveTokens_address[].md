# Function: proposeAddIncentiveTokens(address[])
...
function proposeAddIncentiveTokens(address[] memory tokens) external onlyRole(_REGISTRY_ADMIN_ROLE) {
    ...
    emit WhitelistedIncentiveTokensProposed(_proposedWhitelistedIncentiveTokens.values(), _proposedAddWhitelistedIncentiveTokensEffectiveTime);
}
```
...
```
┌─ [0] ⚙️ FUNCTION: SuperRegistry.proposeAddIncentiveTokens(address[]) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  ├─ [1] ⚙️ FUNCTION: EnumerableSet.add(struct EnumerableSet.AddressSet,address) (NodeID: 1)
  │   💬 Args: [_proposedWhitelistedIncentiveTokens, tokens[i]]
  │   👁️  Def: internal
