# Function: test_ReactivationErrors()
...
- **SuperAsset::activateERC20(address)**
...
```
┌─ [0] ⚙️ FUNCTION: SuperAssetTest.test_ReactivationErrors() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
  ├─ [1] ⚙️ FUNCTION: StdCheats.deal(address,address,uint256) (NodeID: 1)
  │   💬 Args: [address(testToken), address(superAsset), 100e18]
  │   👁️  Def: internal
  ...
  └─ [1] ⚙️ FUNCTION: StdAssertions.assertTrue(bool,string) (NodeID: 99)
      💬 Args: [tokenData.isActive, "Token should be active after reactivation"]
      👁️  Def: internal
```
