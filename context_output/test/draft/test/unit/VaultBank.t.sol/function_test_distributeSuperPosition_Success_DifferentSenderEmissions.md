# Function: test_distributeSuperPosition_Success_DifferentSenderEmissions()
...
function test_distributeSuperPosition_Success_DifferentSenderEmissions() public {
    ...
    vm.mockCall(address(vaultBank), abi.encodeWithSignature("_retrieveSuperPosition(uint64,address,string,string,uint8)", DST_CHAIN_ID, address(token), "Test Token", "TT", 18), abi.encode(superPositionAddress));
    vm.mockCall(address(vaultBank), abi.encodeWithSignature("_mintSP(address,address,uint256)", account, superPositionAddress, amount), abi.encode());
...
```
┌─ [0] ⚙️ FUNCTION: VaultBankTest.test_distributeSuperPosition_Success_DifferentSenderEmissions() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
