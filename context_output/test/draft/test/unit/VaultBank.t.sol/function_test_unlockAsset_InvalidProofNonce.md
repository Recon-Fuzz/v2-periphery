# Function: test_unlockAsset_InvalidProofNonce()
...
function test_unlockAsset_InvalidProofNonce() public {
    ...
    mockProver.mockSuperpositionsBurnedEvent(user, address(token), lockAmount / 2, CURRENT_CHAIN_ID, 0, uint32(DST_CHAIN_ID), yieldSourceOracleId);
...
```
┌─ [0] ⚙️ FUNCTION: VaultBankTest.test_unlockAsset_InvalidProofNonce() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
```
