# Function: property_assetBacking()
...
function property_assetBacking() public {
    ...
    if (superVault.totalSupply() > 0) {
        gt(summedTotalAssets, 0, "if totalSupply > 0, then totalAssets > 0");
    }
}
```
...
### gt(uint256,uint256,string)

- **Kind**: internal
- **Source**: 185:121:68
- **Link**: `lib/setup-helpers/lib/chimera/src/FoundryAsserts.sol:FoundryAsserts:gt(uint256,uint256,string)`

```solidity
function gt(uint256 a, uint256 b, string memory reason) virtual override internal {
    assertGt(a, b, reason);
}
```
...
```
┌─ [0] ⚙️ FUNCTION: Properties.property_assetBacking() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
  ...
  │         💬 Args: [set._inner]
  │         👁️  Def: private
  └─ [1] ⚙️ FUNCTION: FoundryAsserts.gt(uint256,uint256,string) (NodeID: 5)
      💬 Args: [summedTotalAssets, 0, "if totalSupply > 0, then totalAssets > 0"]
      👁️  Def: internal
    └─ [2] ⚙️ FUNCTION: StdAssertions.assertGt(uint256,uint256,string) (NodeID: 6)
        💬 Args: [a, b, reason]
        👁️  Def: internal
```
