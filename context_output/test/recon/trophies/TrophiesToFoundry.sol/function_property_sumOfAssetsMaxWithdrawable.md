# Function: property_sumOfAssetsMaxWithdrawable()
...
function property_sumOfAssetsMaxWithdrawable() public {
    ...
    if (summedTotalAssets == 0) {
        ...
        eq(maxWithdraw, 0, "if sum of assets in SuperVaultStrategy and yield strategies is 0, maxWithdraw should be 0");
    }
}
```
...
```
┌─ [0] ⚙️ FUNCTION: Properties.property_sumOfAssetsMaxWithdrawable() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
  ...
  │   💬 Args: [no args]
  │   👁️  Def: internal
  └─ [1] ⚙️ FUNCTION: FoundryAsserts.eq(uint256,uint256,string) (NodeID: 6)
      💬 Args: [maxWithdraw, 0, "if sum of assets in SuperVaultStrategy and yield strategies is 0, maxWithdraw should be 0"]
      👁️  Def: internal
    └─ [2] ⚙️ FUNCTION: StdAssertions.assertEq(uint256,uint256,string) (NodeID: 7)
        💬 Args: [a, b, reason]
        👁️  Def: internal
```
