...
function forwardPPS(ForwardPPSArgs calldata args) external onlyPPSOracle() {
    ...
    for (uint256 i; i < strategiesLength; ++i) {
        ...
        _forwardPPS(PPSUpdateData({strategy: strategy, isExempt: (!paymentsEnabled) || (upkeepCost == 0), pps: args.ppss[i], timestamp: ts, upkeepCost: upkeepCost}));
    }
}
```
...
- **Kind**: internal
- **Source**: 48614:5428:634
- **Link**: `test/recon/helpers/UnsafeSuperVaultAggregator.sol:UnsafeSuperVaultAggregator:_forwardPPS(struct ISuperVaultAggregator.PPSUpdateData)`
...
function _forwardPPS(PPSUpdateData memory args) internal {
    uint256 minInterval = Math.min(_strategyData[args.strategy].minUpdateInterval, _strategyData[args.strategy].maxStaleness);
...
- **Kind**: modifier
- **Source**: 3875:69:634
- **Link**: `test/recon/helpers/UnsafeSuperVaultAggregator.sol:UnsafeSuperVaultAggregator:onlyPPSOracle()`
...
- **Kind**: internal
- **Source**: 3950:164:634
- **Link**: `test/recon/helpers/UnsafeSuperVaultAggregator.sol:UnsafeSuperVaultAggregator:_onlyPPSOracle()`
...
```
┌─ [0] ⚙️ FUNCTION: UnsafeSuperVaultAggregator.forwardPPS(struct ISuperVaultAggregator.ForwardPPSArgs) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  ...
  │     👁️  Def: private
  ├─ [1] ⚙️ FUNCTION: UnsafeSuperVaultAggregator._forwardPPS(struct ISuperVaultAggregator.PPSUpdateData) (NodeID: 3)
  │   💬 Args: [PPSUpdateData({strategy: strategy, isExempt: (!paymentsEnabled) || (upkeepCost == 0), pps: args.ppss[i], timestamp: ts, upkeepCost: upkeepCost})]
  │   👁️  Def: internal
  │ ├─ [2] ⚙️ FUNCTION: Math.min(uint256,uint256) (NodeID: 4)
  │ │   💬 Args: [_strategyData[args.strategy].minUpdateInterval, _strategyData[args.strategy].maxStaleness]
  │ │   👁️  Def: internal
  ...
  │           💬 Args: [condition]
  │           👁️  Def: internal
  └─ [1] 🔒 MODIFIER: UnsafeSuperVaultAggregator.onlyPPSOracle() (NodeID: 12)
      💬 Args: [no args]
    └─ [2] ⚙️ FUNCTION: UnsafeSuperVaultAggregator._onlyPPSOracle() (NodeID: 13)
        💬 Args: [no args]
        👁️  Def: internal
```
