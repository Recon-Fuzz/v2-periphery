# Function: property_sumOfClaimable()
...
function property_sumOfClaimable() public {
    ...
    if (sumPending == 0) {
        lte(sumClaimable, strategyBalance, "sum of all claimable is > balance available");
    }
}
```
...
```
┌─ [0] ⚙️ FUNCTION: Properties.property_sumOfClaimable() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
  ...
  │       💬 Args: [set._inner]
  │       👁️  Def: private
  └─ [1] ⚙️ FUNCTION: FoundryAsserts.lte(uint256,uint256,string) (NodeID: 4)
      💬 Args: [sumClaimable, strategyBalance, "sum of all claimable is > balance available"]
      👁️  Def: internal
    └─ [2] ⚙️ FUNCTION: StdAssertions.assertLe(uint256,uint256,string) (NodeID: 5)
        💬 Args: [a, b, reason]
        👁️  Def: internal
```
