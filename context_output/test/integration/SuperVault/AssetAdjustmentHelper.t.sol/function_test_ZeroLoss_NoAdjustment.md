# Function: test_ZeroLoss_NoAdjustment()
...
```
┌─ [0] ⚙️ FUNCTION: AssetAdjustmentHelperTest.test_ZeroLoss_NoAdjustment() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
  ...
  │   💬 Args: [fulfillRedeemTotalAssetsOut[0], theoretical[0], "No adjustment needed when no loss"]
  │   👁️  Def: internal
  └─ [1] ⚙️ FUNCTION: StdAssertions.assertEq(uint256,uint256,string) (NodeID: 17)
      💬 Args: [fulfillRedeemTotalAssetsOut[1], theoretical[1], "No adjustment needed when no loss"]
      👁️  Def: internal
```
