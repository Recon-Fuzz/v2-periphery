...
function test_Redeem() public {
    ...
    vm.prank(accountEth);
    uint256 assetsRedeemed = vault.redeem(maxRedeem, accountEth, accountEth);
    assertApproxEqAbs(assetsRedeemed, claimableAssets, 5, "Wrong redeem amount (with tolerance)");
    assertApproxEqAbs(asset.balanceOf(accountEth), initialAssetBalance + claimableAssets, 5, "Wrong final asset balance (with tolerance)");
}
```
...
```
┌─ [0] ⚙️ FUNCTION: SuperVaultTest.test_Redeem() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
  ...
  │             💬 Args: [_sendLogPayloadView]
  │             👁️  Def: internal
  ├─ [1] ⚙️ FUNCTION: StdAssertions.assertApproxEqAbs(uint256,uint256,uint256,string) (NodeID: 514)
  │   💬 Args: [assetsRedeemed, claimableAssets, 5, "Wrong redeem amount (with tolerance)"]
  │   👁️  Def: internal
  └─ [1] ⚙️ FUNCTION: StdAssertions.assertApproxEqAbs(uint256,uint256,uint256,string) (NodeID: 515)
      💬 Args: [asset.balanceOf(accountEth), initialAssetBalance + claimableAssets, 5, "Wrong final asset balance (with tolerance)"]
      👁️  Def: internal
```
