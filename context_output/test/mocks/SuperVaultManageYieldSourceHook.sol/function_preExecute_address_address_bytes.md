...
- **Kind**: internal
- **Source**: 3051:123:611
- **Link**: `test/mocks/SuperVaultManageYieldSourceHook.sol:SuperVaultManageYieldSourceHook:_preExecute(address,address,bytes)`

```solidity
function _preExecute(address, address account, bytes calldata) override internal {
...
┌─ [0] ⚙️ FUNCTION: BaseHook.preExecute(address,address,bytes) (NodeID: 0)
  ...
  │     💬 Args: [context, PRE_EXECUTE_MUTEX_OFFSET]
  │     👁️  Def: private
  └─ [1] ⚙️ FUNCTION: SuperVaultManageYieldSourceHook._preExecute(address,address,bytes) (NodeID: 7)
