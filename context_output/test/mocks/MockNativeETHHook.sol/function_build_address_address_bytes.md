...
- **Kind**: internal
- **Source**: 2763:1148:599
- **Link**: `test/mocks/MockNativeETHHook.sol:MockNativeETHHook:_buildHookExecutions(address,address,bytes)`
...
function _buildHookExecutions(address prevHook, address account, bytes calldata data) override internal view returns (Execution[] memory executions) {
    ...
    executions[0] = Execution({target: yieldSource, value: ethValue, callData: abi.encodeWithSignature("execute()")});
}
```
...
┌─ [0] ⚙️ FUNCTION: BaseHook.build(address,address,bytes) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: MockNativeETHHook._buildHookExecutions(address,address,bytes) (NodeID: 1)
