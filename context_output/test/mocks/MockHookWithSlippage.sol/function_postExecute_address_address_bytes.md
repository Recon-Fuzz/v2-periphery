...
- **Kind**: internal
- **Source**: 1819:137:595
- **Link**: `test/mocks/MockHookWithSlippage.sol:MockHookWithSlippage:_postExecute(address,address,bytes)`

```solidity
/// @notice Override _postExecute (required by BaseHook)
function _postExecute(address, address, bytes calldata) override internal {}
```
...
┌─ [0] ⚙️ FUNCTION: BaseHook.postExecute(address,address,bytes) (NodeID: 0)
  ...
  │     💬 Args: [context, POST_EXECUTE_MUTEX_OFFSET]
  │     👁️  Def: private
  └─ [1] ⚙️ FUNCTION: MockHookWithSlippage._postExecute(address,address,bytes) (NodeID: 7)
