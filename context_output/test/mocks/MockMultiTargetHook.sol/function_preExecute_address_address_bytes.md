...
- **Kind**: internal
- **Source**: 3800:190:597
- **Link**: `test/mocks/MockMultiTargetHook.sol:MockMultiTargetHook:_preExecute(address,address,bytes)`

```solidity
/// @notice Override _preExecute
function _preExecute(address prevHook, address sender, bytes calldata data) override internal {
...
┌─ [0] ⚙️ FUNCTION: BaseHook.preExecute(address,address,bytes) (NodeID: 0)
  ...
  │     💬 Args: [context, PRE_EXECUTE_MUTEX_OFFSET]
  │     👁️  Def: private
  └─ [1] ⚙️ FUNCTION: MockMultiTargetHook._preExecute(address,address,bytes) (NodeID: 7)
