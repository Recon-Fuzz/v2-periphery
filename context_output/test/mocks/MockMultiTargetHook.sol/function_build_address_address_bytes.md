...
- **Kind**: internal
- **Source**: 2236:913:597
- **Link**: `test/mocks/MockMultiTargetHook.sol:MockMultiTargetHook:_buildHookExecutions(address,address,bytes)`
...
function _buildHookExecutions(address, address, bytes calldata) override internal view returns (Execution[] memory) {
    ...
    executions[0] = Execution({target: token, value: 0, callData: abi.encodeWithSelector(IERC20.transfer.selector, recipient1, amount)});
    executions[1] = Execution({target: token, value: 0, callData: abi.encodeWithSelector(IERC20.transfer.selector, recipient2, amount)});
    executions[2] = Execution({target: token, value: 0, callData: abi.encodeWithSelector(IERC20.transfer.selector, recipient3, amount)});
    return executions;
}
```
...
┌─ [0] ⚙️ FUNCTION: BaseHook.build(address,address,bytes) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: MockMultiTargetHook._buildHookExecutions(address,address,bytes) (NodeID: 1)
