# Function: test_DecimalsGasConsumer_ReturnsZeroWithRevertOnErrorFalse()
...
function test_DecimalsGasConsumer_ReturnsZeroWithRevertOnErrorFalse() public {
    ...
    MockAggregatorGasConsumerOnDecimals gasConsumingDecimalsFeed = new MockAggregatorGasConsumerOnDecimals(int256(INITIAL_PRICE));
...
- **MockAggregatorGasConsumerOnDecimals::setUpdatedAt(uint256)**
...
```
┌─ [0] ⚙️ FUNCTION: SuperOracleL2Test.test_DecimalsGasConsumer_ReturnsZeroWithRevertOnErrorFalse() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
  └─ [1] ⚙️ FUNCTION: StdAssertions.assertGt(uint256,uint256,string) (NodeID: 1)
      💬 Args: [quoteAmount, 0, "Should get quote from working provider despite gas-consuming decimals feed"]
      👁️  Def: internal
```
