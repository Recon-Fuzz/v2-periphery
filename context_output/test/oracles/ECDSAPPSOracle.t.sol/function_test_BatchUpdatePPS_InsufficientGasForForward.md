# Function: test_BatchUpdatePPS_InsufficientGasForForward()
...
function test_BatchUpdatePPS_InsufficientGasForForward() public {
    ...
    vm.prank(user);
    oracleECDSA.updatePPS{gas: 1_000_000}(IECDSAPPSOracle.UpdatePPSArgs({strategies: data.strategies, proofsArray: data.proofsArray, ppss: data.ppss, timestamps: data.timestamps}));
}
```
...
```
┌─ [0] ⚙️ FUNCTION: ECDSAPPSOracleTest.test_BatchUpdatePPS_InsufficientGasForForward() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
