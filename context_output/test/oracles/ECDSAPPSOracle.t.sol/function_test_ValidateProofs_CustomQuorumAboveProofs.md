# Function: test_ValidateProofs_CustomQuorumAboveProofs()
...
function test_ValidateProofs_CustomQuorumAboveProofs() public {
    ...
    oracleECDSA.validateProofs(IECDSAPPSOracle.ValidationParams({strategy: address(svStrategy), proofs: proofs, pps: PPS, timestamp: block.timestamp}), 3);
}
```
...
- **ECDSAPPSOracle::validateProofs(struct IECDSAPPSOracle.ValidationParams,uint256)**
...
```
┌─ [0] ⚙️ FUNCTION: ECDSAPPSOracleTest.test_ValidateProofs_CustomQuorumAboveProofs() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
