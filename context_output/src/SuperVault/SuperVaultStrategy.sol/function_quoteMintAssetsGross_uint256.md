# Function: quoteMintAssetsGross(uint256)
...
function quoteMintAssetsGross(uint256 shares) external view returns (uint256 assetsGross, uint256 assetsNet) {
    ...
    if (feeBps >= BPS_PRECISION) revert INVALID_AMOUNT();
...
```
┌─ [0] ⚙️ FUNCTION: SuperVaultStrategy.quoteMintAssetsGross(uint256) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  ...
  │   💬 Args: [shares, pps, PRECISION, Math.Rounding.Ceil]
  │   👁️  Def: internal
  ...
  └─ [1] ⚙️ FUNCTION: Math.mulDiv(uint256,uint256,uint256,enum Math.Rounding) (NodeID: 11)
      💬 Args: [assetsNet, BPS_PRECISION, (BPS_PRECISION - feeBps), Math.Rounding.Ceil]
      👁️  Def: internal
