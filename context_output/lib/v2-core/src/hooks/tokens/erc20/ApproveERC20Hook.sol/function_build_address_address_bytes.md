...
- **Kind**: internal
- **Source**: 1386:1033:396
- **Link**: `lib/v2-core/src/hooks/tokens/erc20/ApproveERC20Hook.sol:ApproveERC20Hook:_buildHookExecutions(address,address,bytes)`
...
function _buildHookExecutions(address prevHook, address account, bytes calldata data) override internal view returns (Execution[] memory executions) {
    ...
    executions[1] = Execution({target: token, value: 0, callData: abi.encodeCall(IERC20.approve, (spender, amount))});
}
```
...
┌─ [0] ⚙️ FUNCTION: BaseHook.build(address,address,bytes) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: ApproveERC20Hook._buildHookExecutions(address,address,bytes) (NodeID: 1)
    ...
    │   💬 Args: [data, 40]
    │   👁️  Def: internal
    └─ [2] ⚙️ FUNCTION: BaseHook._decodeBool(bytes,uint256) (NodeID: 5)
