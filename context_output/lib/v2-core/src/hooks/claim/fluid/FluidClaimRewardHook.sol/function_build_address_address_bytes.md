...
- **Kind**: internal
- **Source**: 1616:434:371
- **Link**: `lib/v2-core/src/hooks/claim/fluid/FluidClaimRewardHook.sol:FluidClaimRewardHook:_buildHookExecutions(address,address,bytes)`
...
┌─ [0] ⚙️ FUNCTION: BaseHook.build(address,address,bytes) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: FluidClaimRewardHook._buildHookExecutions(address,address,bytes) (NodeID: 1)
    ...
    └─ [2] ⚙️ FUNCTION: BaseClaimRewardHook._build(address,bytes) (NodeID: 4)
        💬 Args: [stakingRewards, abi.encodeCall(IFluidLendingStakingRewards.getReward, ())]
        👁️  Def: internal
```
