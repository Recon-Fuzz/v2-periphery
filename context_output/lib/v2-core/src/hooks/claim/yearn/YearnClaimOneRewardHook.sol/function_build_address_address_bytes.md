...
- **Kind**: internal
- **Source**: 1615:526:374
- **Link**: `lib/v2-core/src/hooks/claim/yearn/YearnClaimOneRewardHook.sol:YearnClaimOneRewardHook:_buildHookExecutions(address,address,bytes)`
...
┌─ [0] ⚙️ FUNCTION: BaseHook.build(address,address,bytes) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: YearnClaimOneRewardHook._buildHookExecutions(address,address,bytes) (NodeID: 1)
    ...
    │   💬 Args: [data, 52]
    │   👁️  Def: internal
    └─ [2] ⚙️ FUNCTION: BaseClaimRewardHook._build(address,bytes) (NodeID: 5)
        💬 Args: [yieldSource, abi.encodeCall(IYearnStakingRewardsMulti.getOneReward, (rewardToken))]
        👁️  Def: internal
```
