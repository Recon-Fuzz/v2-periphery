...
- **Kind**: internal
- **Source**: 1524:864:384
- **Link**: `lib/v2-core/src/hooks/stake/gearbox/GearboxStakeHook.sol:GearboxStakeHook:_buildHookExecutions(address,address,bytes)`
...
function _buildHookExecutions(address prevHook, address account, bytes calldata data) override internal view returns (Execution[] memory executions) {
    ...
    executions[0] = Execution({target: yieldSource, value: 0, callData: abi.encodeCall(IGearboxFarmingPool.deposit, (amount))});
}
```
...
- **Kind**: internal
- **Source**: 3700:161:384
- **Link**: `lib/v2-core/src/hooks/stake/gearbox/GearboxStakeHook.sol:GearboxStakeHook:_decodeAmount(bytes)`
...
┌─ [0] ⚙️ FUNCTION: BaseHook.build(address,address,bytes) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: GearboxStakeHook._buildHookExecutions(address,address,bytes) (NodeID: 1)
    ...
    │     💬 Args: [data, 32]
    │     👁️  Def: internal
    ├─ [2] ⚙️ FUNCTION: GearboxStakeHook._decodeAmount(bytes) (NodeID: 4)
    │   💬 Args: [data]
    │   👁️  Def: private
