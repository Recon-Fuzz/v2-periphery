...
- **Kind**: internal
- **Source**: 1551:880:382
- **Link**: `lib/v2-core/src/hooks/stake/fluid/FluidUnstakeHook.sol:FluidUnstakeHook:_buildHookExecutions(address,address,bytes)`
...
function _buildHookExecutions(address prevHook, address account, bytes calldata data) override internal view returns (Execution[] memory executions) {
    ...
    executions[0] = Execution({target: yieldSource, value: 0, callData: abi.encodeCall(IFluidLendingStakingRewards.withdraw, (amount))});
}
```
...
- **Kind**: internal
- **Source**: 3945:138:382
- **Link**: `lib/v2-core/src/hooks/stake/fluid/FluidUnstakeHook.sol:FluidUnstakeHook:_decodeAmount(bytes)`
...
┌─ [0] ⚙️ FUNCTION: BaseHook.build(address,address,bytes) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: FluidUnstakeHook._buildHookExecutions(address,address,bytes) (NodeID: 1)
    ...
    │     💬 Args: [data, 32]
    │     👁️  Def: internal
    ├─ [2] ⚙️ FUNCTION: FluidUnstakeHook._decodeAmount(bytes) (NodeID: 4)
    │   💬 Args: [data]
    │   👁️  Def: private
