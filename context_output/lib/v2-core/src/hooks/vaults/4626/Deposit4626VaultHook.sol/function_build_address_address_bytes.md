...
- **Kind**: internal
- **Source**: 1678:830:400
- **Link**: `lib/v2-core/src/hooks/vaults/4626/Deposit4626VaultHook.sol:Deposit4626VaultHook:_buildHookExecutions(address,address,bytes)`
...
function _buildHookExecutions(address prevHook, address account, bytes calldata data) override internal view returns (Execution[] memory executions) {
    ...
    executions[0] = Execution({target: yieldSource, value: 0, callData: abi.encodeCall(IERC4626.deposit, (amount, account))});
}
```
...
- **Kind**: internal
- **Source**: 4064:138:400
- **Link**: `lib/v2-core/src/hooks/vaults/4626/Deposit4626VaultHook.sol:Deposit4626VaultHook:_decodeAmount(bytes)`
...
┌─ [0] ⚙️ FUNCTION: BaseHook.build(address,address,bytes) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: Deposit4626VaultHook._buildHookExecutions(address,address,bytes) (NodeID: 1)
    ...
    │     💬 Args: [data, 32]
    │     👁️  Def: internal
    ├─ [2] ⚙️ FUNCTION: Deposit4626VaultHook._decodeAmount(bytes) (NodeID: 4)
    │   💬 Args: [data]
    │   👁️  Def: private
