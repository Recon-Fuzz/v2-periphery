...
- **Kind**: internal
- **Source**: 1830:1093:403
- **Link**: `lib/v2-core/src/hooks/vaults/5115/Deposit5115VaultHook.sol:Deposit5115VaultHook:_buildHookExecutions(address,address,bytes)`
...
function _buildHookExecutions(address prevHook, address account, bytes calldata data) override internal view returns (Execution[] memory executions) {
    ...
    executions[0] = Execution({target: yieldSource, value: (tokenIn == address(0)) ? amount : 0, callData: abi.encodeCall(IStandardizedYield.deposit, (account, tokenIn, amount, minSharesOut))});
}
```
...
┌─ [0] ⚙️ FUNCTION: BaseHook.build(address,address,bytes) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: Deposit5115VaultHook._buildHookExecutions(address,address,bytes) (NodeID: 1)
