...
- **Kind**: internal
- **Source**: 1184:576:407
- **Link**: `lib/v2-core/src/hooks/vaults/7540/CancelRedeemRequest7540Hook.sol:CancelRedeemRequest7540Hook:_buildHookExecutions(address,address,bytes)`
...
function _buildHookExecutions(address, address account, bytes calldata data) override internal pure returns (Execution[] memory executions) {
    ...
    executions[0] = Execution({target: yieldSource, value: 0, callData: abi.encodeCall(IERC7540CancelRedeem.cancelRedeemRequest, (0, account))});
}
```
...
┌─ [0] ⚙️ FUNCTION: BaseHook.build(address,address,bytes) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: CancelRedeemRequest7540Hook._buildHookExecutions(address,address,bytes) (NodeID: 1)
