...
- **Kind**: internal
- **Source**: 1744:896:410
- **Link**: `lib/v2-core/src/hooks/vaults/7540/Deposit7540VaultHook.sol:Deposit7540VaultHook:_buildHookExecutions(address,address,bytes)`
...
function _buildHookExecutions(address prevHook, address account, bytes calldata data) override internal view returns (Execution[] memory executions) {
    ...
    executions[0] = Execution({target: yieldSource, value: 0, callData: abi.encodeCall(IERC7540.deposit, (amount, account, account))});
}
```
...
- **Kind**: internal
- **Source**: 4214:138:410
- **Link**: `lib/v2-core/src/hooks/vaults/7540/Deposit7540VaultHook.sol:Deposit7540VaultHook:_decodeAmount(bytes)`
...
┌─ [0] ⚙️ FUNCTION: BaseHook.build(address,address,bytes) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: Deposit7540VaultHook._buildHookExecutions(address,address,bytes) (NodeID: 1)
    ...
    │     💬 Args: [data, 32]
    │     👁️  Def: internal
    ├─ [2] ⚙️ FUNCTION: Deposit7540VaultHook._decodeAmount(bytes) (NodeID: 4)
    │   💬 Args: [data]
    │   👁️  Def: private
