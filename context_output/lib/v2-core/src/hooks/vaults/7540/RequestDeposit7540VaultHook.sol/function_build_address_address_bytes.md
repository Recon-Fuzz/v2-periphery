...
- **Kind**: internal
- **Source**: 1739:903:412
- **Link**: `lib/v2-core/src/hooks/vaults/7540/RequestDeposit7540VaultHook.sol:RequestDeposit7540VaultHook:_buildHookExecutions(address,address,bytes)`
...
function _buildHookExecutions(address prevHook, address account, bytes calldata data) override internal view returns (Execution[] memory executions) {
    ...
    executions[0] = Execution({target: yieldSource, value: 0, callData: abi.encodeCall(IERC7540.requestDeposit, (amount, account, account))});
}
```
...
- **Kind**: internal
- **Source**: 4287:138:412
- **Link**: `lib/v2-core/src/hooks/vaults/7540/RequestDeposit7540VaultHook.sol:RequestDeposit7540VaultHook:_decodeAmount(bytes)`
...
┌─ [0] ⚙️ FUNCTION: BaseHook.build(address,address,bytes) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: RequestDeposit7540VaultHook._buildHookExecutions(address,address,bytes) (NodeID: 1)
    ...
    │     💬 Args: [data, 32]
    │     👁️  Def: internal
    ├─ [2] ⚙️ FUNCTION: RequestDeposit7540VaultHook._decodeAmount(bytes) (NodeID: 4)
    │   💬 Args: [data]
    │   👁️  Def: private
