...
- **Kind**: internal
- **Source**: 1690:896:416
- **Link**: `lib/v2-core/src/hooks/vaults/ethena/EthenaCooldownSharesHook.sol:EthenaCooldownSharesHook:_buildHookExecutions(address,address,bytes)`
...
function _buildHookExecutions(address prevHook, address account, bytes calldata data) override internal view returns (Execution[] memory executions) {
    ...
    executions[0] = Execution({target: yieldSource, value: 0, callData: abi.encodeCall(IStakedUSDeCooldown.cooldownShares, (shares))});
}
```
...
- **Kind**: internal
- **Source**: 3841:138:416
- **Link**: `lib/v2-core/src/hooks/vaults/ethena/EthenaCooldownSharesHook.sol:EthenaCooldownSharesHook:_decodeAmount(bytes)`
...
┌─ [0] ⚙️ FUNCTION: BaseHook.build(address,address,bytes) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: EthenaCooldownSharesHook._buildHookExecutions(address,address,bytes) (NodeID: 1)
    ...
    │     💬 Args: [data, 32]
    │     👁️  Def: internal
    ├─ [2] ⚙️ FUNCTION: EthenaCooldownSharesHook._decodeAmount(bytes) (NodeID: 4)
    │   💬 Args: [data]
    │   👁️  Def: private
