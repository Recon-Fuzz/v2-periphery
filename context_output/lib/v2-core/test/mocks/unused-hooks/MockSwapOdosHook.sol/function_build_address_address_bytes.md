...
- **Kind**: internal
- **Source**: 2253:1257:497
- **Link**: `lib/v2-core/test/mocks/unused-hooks/MockSwapOdosHook.sol:MockSwapOdosHook:_buildHookExecutions(address,address,bytes)`
...
function _buildHookExecutions(address prevHook, address account, bytes calldata data) override internal view returns (Execution[] memory executions) {
    ...
    executions[0] = Execution({target: address(odosRouterV2), value: (inputToken == address(0)) ? inputAmount : 0, callData: abi.encodeCall(IOdosRouterV2.swap, (_getSwapInfo(account, prevHook, data), pathDefinition, executor, referralCode))});
}
```
...
- **Kind**: internal
- **Source**: 4953:913:497
- **Link**: `lib/v2-core/test/mocks/unused-hooks/MockSwapOdosHook.sol:MockSwapOdosHook:_getSwapInfo(address,address,bytes)`
...
┌─ [0] ⚙️ FUNCTION: BaseHook.build(address,address,bytes) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: MockSwapOdosHook._buildHookExecutions(address,address,bytes) (NodeID: 1)
    ...
    │   👁️  Def: internal
    └─ [2] ⚙️ FUNCTION: MockSwapOdosHook._getSwapInfo(address,address,bytes) (NodeID: 9)
