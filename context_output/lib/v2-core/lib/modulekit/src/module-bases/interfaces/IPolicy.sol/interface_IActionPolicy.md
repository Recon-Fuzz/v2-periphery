...
- **Documentation**:  IActionPolicy is a policy that enforces restrictions on actions. It is called during the
   validation phase
...
### checkAction(ConfigId,address,address,uint256,bytes)

- **Signature**: `checkAction(ConfigId,address,address,uint256,bytes)`
- **Visibility**: external
- **Source Range**: 3136:192:217

**Signature:**
```solidity
function checkAction(ConfigId id, address account, address target, uint256 value, bytes calldata data) external returns (uint256);;
```
