...
- **Signature**: `checkSignaturePolicy(bytes32,address,bytes32,bytes)`
- **Visibility**: external
- **Source Range**: 3547:186:149

**Signature:**
```solidity
function checkSignaturePolicy(bytes32 id, address sender, bytes32 hash, bytes calldata sig) external view returns (uint256);;
```
