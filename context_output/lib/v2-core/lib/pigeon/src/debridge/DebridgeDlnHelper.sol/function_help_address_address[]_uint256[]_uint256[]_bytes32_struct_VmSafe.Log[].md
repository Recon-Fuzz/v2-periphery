...
function help(address dlnSource, address[] memory dlnDestinations, uint256[] memory forkIds, uint256[] memory destinationChainIds, bytes32 eventSelector, Vm.Log[] calldata logs) external {
    ...
    for (uint256 i; i < chains; ) {
        _help(HelpArgs({dlnSource: dlnSource, dlnDestination: dlnDestinations[i], forkId: forkIds[i], destinationChainId: destinationChainIds[i], eventSelector: eventSelector, logs: logs}));
        unchecked {
            ++i;
        }
    }
...
┌─ [0] ⚙️ FUNCTION: DebridgeDlnHelper.help(address,address[],uint256[],uint256[],bytes32,struct VmSafe.Log[]) (NodeID: 0)
  ...
  └─ [1] ⚙️ FUNCTION: DebridgeDlnHelper._help(struct DebridgeDlnHelper.HelpArgs) (NodeID: 1)
      💬 Args: [HelpArgs({dlnSource: dlnSource, dlnDestination: dlnDestinations[i], forkId: forkIds[i], destinationChainId: destinationChainIds[i], eventSelector: eventSelector, logs: logs})]
      👁️  Def: internal
