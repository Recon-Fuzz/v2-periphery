# Function: help(address,address[],uint256[],uint256[],struct VmSafe.Log[])
...
function help(address dlnSource, address[] memory dlnDestinations, uint256[] memory forkIds, uint256[] memory destinationChainIds, Vm.Log[] calldata logs) external {
    ...
    for (uint256 i; i < chains; ) {
        _help(HelpArgs({dlnSource: dlnSource, dlnDestination: dlnDestinations[i], forkId: forkIds[i], destinationChainId: destinationChainIds[i], eventSelector: DlnOrderCreated, logs: logs}));
        unchecked {
            ++i;
        }
    }
...
```
┌─ [0] ⚙️ FUNCTION: DebridgeDlnHelper.help(address,address[],uint256[],uint256[],struct VmSafe.Log[]) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: DebridgeDlnHelper._help(struct DebridgeDlnHelper.HelpArgs) (NodeID: 1)
      💬 Args: [HelpArgs({dlnSource: dlnSource, dlnDestination: dlnDestinations[i], forkId: forkIds[i], destinationChainId: destinationChainIds[i], eventSelector: DlnOrderCreated, logs: logs})]
      👁️  Def: internal
    └─ [2] ⚙️ FUNCTION: StdCheats.deal(address,address,uint256) (NodeID: 2)
        💬 Args: [vars.tokenAddress, vars.takerAddress, vars.fulfillAmount]
        👁️  Def: internal
